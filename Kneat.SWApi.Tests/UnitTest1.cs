...
namespace Kneat.SWApi.Tests
{
    public class UnitTest1
    {
        ...
        [InlineData("6 years")]
        [InlineData("12 years")]
        public void TestConvertYear(string consumers)
        {
            var infoMock = new Mock<ISWApiInfrastructure>();

            var time = Convert.ToInt16(consumers.Split(' ')[0]);
            var expected = time * 24 * 365;

            var result = new StarShipInformation(infoMock.Object).convertTimeStringInIntegerValue(consumers);
